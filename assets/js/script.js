...
const SUPABASE_URL = "https://qhgebxbxzhncgmrvhxye.supabase.co"; 
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFoZ2VieGJ4emhuY2dtcnZoeHllIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc5OTMxOTAsImV4cCI6MjA5MzU2OTE5MH0.DRQlVKSnrRmz5VpfbwnQmJKV4Ns05W7P_yOROGFC6Mc";
...
function dadosClienteFormulario(prefixo) {
    ...
    return {
        nome: limparValorNulo(document.getElementById(`${prefixo}-nome`)?.value) || "",
...
// ==========================================
function abrirModalAtendimento(nomePerfume) {
...
async function confirmarWhatsApp() {
    ...
    const mensagemText = `Olá! Meu nome é ${cliente.nome}. Gostaria de consultar a disponibilidade do perfume ${perfumeSelecionadoParaWhatsApp}.`;
    ...
    return `
        <div class="product-card">
            ...
            <div class="product-card-body">
                ...
                ${volumesDisplay}
                ${notasDisplay}
                <button class="btn" onclick="abrirModalAtendimento('${nomeEscapado}')">
...
async function login() {
    ...
    try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/configuracoes?chave=eq.senha_admin`, {
            method: "GET",
            headers: headers
        });
        ...
        if (dados && dados.length > 0 && senhaDigitada === dados[0].valor) {
